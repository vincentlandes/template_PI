...
__kernel void swap_buffers (__global uint* pattern, __global uint* second)
{
	...
	second[i] = pattern [i];
	uint localworksize = get_local_size;
...
__kernel void process_pixels (__global uint* pattern, __global uint* second, uint pw)
{
	...
	uint n = GetBit(x - 1, y - 1, pw, second) + GetBit(x, y - 1, pw, second) + GetBit(x + 1, y - 1, pw, second) + GetBit(x - 1, y, pw, second) + GetBit(x + 1, y, pw, second) + GetBit(x - 1, y + 1, pw, second) + GetBit(x, y + 1, pw, second) + GetBit(x + 1, y + 1, pw, second);
